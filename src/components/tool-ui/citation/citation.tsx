...
export function Citation(props: CitationProps) {
  ...
  if (variant === "inline") {
    return (
      <Popover open={open}>
        <PopoverTrigger render={<button type="button" aria-label={title} data-tool-ui-id={id} data-slot="citation" onClick={handleClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className={cn(
                          ...
                          className,
                        )} />}>{iconElement}<span className="text-muted-foreground">{domain}</span></PopoverTrigger>
        <PopoverContent
          side="top"
          align="start"
